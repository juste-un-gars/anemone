...
for cmd in jq sqlite3 openssl go smbpasswd useradd btrfs smartctl; do
    if ! command -v $cmd &> /dev/null; then
        case $cmd in
            go) MISSING_DEPS+=("golang-go") ;;
            smbpasswd) MISSING_DEPS+=("samba") ;;
            btrfs) MISSING_DEPS+=("btrfs-progs") ;;
            smartctl) MISSING_DEPS+=("smartmontools") ;;
            *) MISSING_DEPS+=("$cmd") ;;
        esac
    fi
done
...
if [ ${#MISSING_DEPS[@]} -ne 0 ]; then
    ...
    case $DISTRO in
        ubuntu|debian|linuxmint)
            apt-get update -qq
            apt-get install -y -qq jq sqlite3 openssl golang-go samba passwd btrfs-progs smartmontools
            ...
            ;;
        fedora|rhel|centos)
            dnf install -y -q jq sqlite openssl golang samba passwd btrfs-progs smartmontools
